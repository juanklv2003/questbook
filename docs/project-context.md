...
- **PDF parsing**: `pdf-parse` in a `worker_threads` worker when a compiled `pdfExtractWorker.js` exists; otherwise (and if the worker fails) extraction runs in-process. The TS worker + `ts-node/register` path is not used — it typechecks without Node types and 422s every upload. `PdfTextExtractor` returns `{ text, totalCharacters }`: the worker clips to `maxChars` **before** posting (so the IPC and heap never hold the 2-3M chars of a 300-page book) while `totalCharacters` stays the real count for the upload budget bar. Buffers go to pdf-parse without copying (pdfjs takes ownership) and a valid-but-empty PDF is NOT re-parsed in-process.
...
- `POST /generate`: Uploads a PDF or text to generate a new deck. Expects `multipart/form-data` with `file` (optional) and `name` (required). If a `file` is provided, its text is extracted with `pdf-parse` (`PdfTextExtractor`) before batched calls to Gemini (`GeminiFlashcardGenerator`). Source text is capped (`PDF_MAX_TEXT_CHARS`, default 1_000_000, hard max 4_000_000 ≈ 1M tokens): `combinePdfUploads` accumulates the PDFs into that budget, stops parsing the ones that no longer fit (`skippedFiles`) and never reads a >7 MB PDF into RAM just to archive it in Cloudinary (`cloudinaryAttachmentMaxBytes`). Every AI call only sees a **window** of `AI_DECK_PROMPT_CHARS_PER_CALL` chars (default 200_000 ≈ 50k tokens) taken in document order by `deckPromptWindow`, so a big document no longer repeats the whole prompt in each batch (bounded tokens/latency per call, TPM-friendly, consecutive batches cover different parts); documents that fit in one call are sent whole exactly as before. Each batch sets Gemini `maxOutputTokens` from `deckBatchMaxOutputTokens` (difficulty-aware). Wall-clock budget is `AI_DECK_TOTAL_TIMEOUT_MS` (default 480s). Each call gets the remaining budget minus `DECK_GENERATION_BATCH_RESERVE_MS` per queued batch, capped at `AI_DECK_TIMEOUT_MS` and never more than the time left. Empty-batch / `MAX_TOKENS` recovery splits a batch in half at most once (`DECK_GENERATION_MAX_SPLIT_DEPTH = 1`); a second truncation parses what Gemini returned. Logs under `[deck-gen]` include `finishReason` and token usage. A prompt blocked by Gemini (`promptFeedback.blockReason`) returns `502 { code: 'AI_CONTENT_BLOCKED' }`. Fallback model chain lives in `core/ai/geminiModels.ts` and only lists models verified live against the API (2.0/1.5 flash are shut down). On late-batch failure, partial decks above ~60% of the requested count (min 8 cards) may still be persisted (502/504/422 and 503 overload — not 429 quota). `GET /api/v1/health` exposes `ai.deckGeneration*TimeoutMs` and `ai.deckPromptCharsPerCall`: the client waits up to `deckGenerationClientTimeoutMs` but never less than the backend budget + upload + PDF-extract slack (the browser must outlast the backend's own 504). Node `requestTimeout`/keep-alive match `AI_DECK_TOTAL_TIMEOUT_MS`. Diagnostic script: `npm run diagnose:deck-gen -- <pdf>` (backend).
